...
export default function AwardsSection() {
  return (
    <section id="certifications" className="scroll-mt-16 bg-acid py-20 text-ink md:py-28">
            ...
            <MotionWrapper key={cert.name + cert.date} delay={index * 0.04}>
              <div className="grid gap-3 py-5 md:grid-cols-[1fr_auto] md:items-center md:gap-8 md:py-6">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm text-ink/65">{cert.issuer}</p>
                </div>
                <div className="flex flex-wrap items-center gap-2 text-sm md:justify-end">
                  <span className="font-semibold text-ink/60">{cert.type}</span>
                  <span className="font-semibold tabular-nums text-ink/60">{cert.date}</span>
