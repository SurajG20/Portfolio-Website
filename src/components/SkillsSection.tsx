...
export default function SkillsSection() {
  return (
    <section id="skills" className="scroll-mt-16 bg-mist py-20 text-foreground md:py-28">
          ...
          {skills.map(({ label, items }, index) => (
            <MotionWrapper key={label} delay={index * 0.05}>
              <div className="grid gap-2 py-5 md:grid-cols-[180px_1fr] md:gap-8 md:py-6">
                <h3 className="text-sm font-black text-muted-foreground">{label}</h3>
                <ul className="flex flex-wrap gap-x-3 gap-y-2 font-display text-xl leading-tight text-foreground md:text-2xl">
                  {items.map((skill) => (
                    <li
                      key={skill}
